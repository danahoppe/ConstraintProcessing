...
public class InstanceCheckerParser {
	...
	private PConstraint parseElementConstraint(String name, String reference, PVariable[] scope, Element parameters) throws FormatException {
		...
		return new PElement(name, reference, scope, index, table.toArray(new Object[table.size()]), value);
	}

	private PConstraint parseWeightedSumConstraint(String name, String reference, PVariable[] scope, Element parameters) throws FormatException {
		...
		return new PWeightedSum(name, reference, scope, coeffs, operator, limit);
	}
	...
	private PConstraint parseCumulativeConstraint(String name, String reference, PVariable[] involvedVariables, Element parameters) throws FormatException {
		...
		if (st.hasMoreTokens())
			throw new FormatException("Too many tokens in the parameters of constraint " + name);
		return new PCumulative(name, reference, involvedVariables, tasks.toArray(new Task[tasks.size()]), limit);
	}
	...
	private PConstraint parseConstraint(Element constraintElement) throws FormatException {
		...
		if (mapOfRelations.containsKey(reference)) {
			...
			return new PExtensionConstraint(name, reference, involvedVariables, mapOfRelations.get(reference));
		}
		...
		if (mapOfFunctions.containsKey(reference)) {
			...
			return new PIntensionConstraint(name, reference, involvedVariables, mapOfFunctions.get(reference), parameters.getTextContent());
		}
		...
		if (mapOfPredicates.containsKey(reference)) {
			...
			checkEffectiveParameters(name, involvedVariables, effectiveParameters);
			return new PIntensionConstraint(name, reference, involvedVariables, mapOfPredicates.get(reference), parameters.getTextContent());
		}
		...
		if (lreference.equals(InstanceTokens.getLowerCaseGlobalNameOf(InstanceTokens.ALL_DIFFERENT)))
			return new PAllDifferent(name, reference, involvedVariables);
		if (lreference.equals(InstanceTokens.getLowerCaseGlobalNameOf(InstanceTokens.ELEMENT)))
			return parseElementConstraint(name, reference, involvedVariables, parameters);
		if (lreference.equals(InstanceTokens.getLowerCaseGlobalNameOf(InstanceTokens.WEIGHTED_SUM)))
			return parseWeightedSumConstraint(name, reference, involvedVariables, parameters);
		if (lreference.equals(InstanceTokens.getLowerCaseGlobalNameOf(InstanceTokens.CUMULATIVE)))
			return parseCumulativeConstraint(name, reference, involvedVariables, parameters);
	...
	private int convert(PIntensionConstraint constraint, int limit) throws FormatException {
		...
		if (!(constraint.getFunction() instanceof PPredicate)) {
			...
			PExtensionConstraint c = new PExtensionConstraint(constraint.getName(), constraint.getReference(), constraint.getScope(), relation);
