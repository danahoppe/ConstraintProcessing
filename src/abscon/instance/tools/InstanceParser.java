...
public class InstanceParser {
	...
	private void parsePresentation(Element presentationElement) {
		...
		presentation = presentationElement.getAttribute(InstanceTokens.NAME.trim());
	...
	private PRelation parseRelationTuples(String name, int arity, int nbTuples, String semantics, String textContent) {
		...
		relation = new PRelation(name, arity, nbTuples, semantics, tuples);
		return relation;
	}
	...
	private PConstraint parseElementConstraint(String name, String reference, PVariable[] scope, Element parameters) {
	...
	private PConstraint parseWeightedSumConstraint(String name, String reference, PVariable[] scope, Element parameters) {
	...
	private PConstraint parseCumulativeConstraint(String name, String reference, PVariable[] scope, Element parameters) {
		...
		return new PCumulative(name, reference, scope, tasks.toArray(new Task[tasks.size()]), limit);
	}
	...
	private PConstraint parseConstraint(Element constraintElement) {
		...
		if (mapOfRelations.containsKey(reference)) {
			...
			return new PExtensionConstraint(name, reference, scope, mapOfRelations.get(reference));
		}
		...
		if (mapOfFunctions.containsKey(reference)) {
			...
			return new PIntensionConstraint(name, reference, scope, mapOfFunctions.get(reference), parameters.getTextContent());
		}

		if (mapOfPredicates.containsKey(reference)) {
			Element parameters = (Element) constraintElement.getElementsByTagName(InstanceTokens.PARAMETERS).item(0);
			nbIntensionConstraints++;
			return new PIntensionConstraint(name, reference, scope, mapOfPredicates.get(reference), parameters.getTextContent());
		}
